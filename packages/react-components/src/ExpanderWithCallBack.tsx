...
function ExpanderWithCallBack ({ children, className = '', isOpen, summary, summaryMeta, summarySub, toggleOpen, withDot, withHidden }: Props): React.ReactElement<Props> {
  ...
  return (
    <div className={`ui--Expander ${isOpen ? 'isExpanded' : ''} ${hasContent ? 'hasContent' : ''} ${className}`}>
      <div
      ...
      >
        <div className='ui--Expander-summary-header'>
          {hasContent
            ? <Icon icon={isOpen ? 'angle-double-down' : 'angle-double-right'} />
            : withDot
              ? <Icon icon='circle' />
              : undefined
...
export default React.memo(styled(ExpanderWithCallBack)`
  max-width: 100%;
  overflow: hidden;
